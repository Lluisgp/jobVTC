...
function jobvtc_admin_front_city() {
    ...
    //Insert form      
    echo '<form method="post" onsubmit="insertItem(this)" ><input type="hidden" id="do_action" value="insert_city">Add a new city, insert the name: </span><span class="input-text-wrap"><input type="text" id="new_city_name" class="ptitle" value=""></span></label><label><span class="title"> insert the DLC </span><span class="input-text-wrap"><input type="text" id="new_city_dlc" class="ptitle" value=""></span></label><input type="submit" id="addCity" class="button action" value="Add"></form>';
//Table Head
    echo '<table class="wp-list-table widefat fixed striped posts"><thead><tr>
    <td id="jobCargoId" class="manage-column column-cb check-column"><input id="cb-select" type="checkbox"></td><th scope="col" id="jobCargoName" class="manage-column column-title column-primary sortable desc"><a href="http://localhost/wordpress/wp-admin/edit.php?orderby=title&amp;order=asc"><span>Name</span><span class="sorting-indicator"></span></a></th><th scope="col" id="jobCargoDLC" class="manage-column column-author">DLC</th></thead>';

    //Row
    echo '<tr><th scope="row" class="check-column">			<label class="screen-reader-text" for="cb-select-7">Elige (sin título)</label>
			<input id="cb-select-7" type="checkbox" name="post[]" value="7">
			<div class="locked-indicator">
				<span class="locked-indicator-icon" aria-hidden="true"></span>
				<span class="screen-reader-text">“(sin título)” está bloqueado</span>
			</div>
		</th></td>
<td class="title column-title has-row-actions column-primary page-title" data-colname="Título"><div class="locked-info"><span class="locked-avatar"></span> <span class="locked-text"></span></div>
<strong><a class="row-title" href="http://localhost/wordpress/wp-admin/post.php?post=7&amp;action=edit" aria-label="“(sin título)” (Editar)">Bourdeaux</a></strong>



<th scope="col" id="1" class="manage-column column-author">vive la france</th>';

    //Table end
    echo '  <tfoot><tr><thead><tr>
...
function jobvtc_admin_front2() {
    ...
    if (get_option("wolfvtc_setup") == FALSE) {
        ...
        if (get_option("wolfvtc_dbversion") < 1) {
            ...
            foreach ($cities as $c) {
                $wpdb->insert(
                        $wpdb->prefix . 'wolfvtc_cities', array(
                    'cityname' => $c,
                        )
                );
            }
            ...
            $wpdb->insert(
                    $wpdb->prefix . 'wolfvtc_cargo', array(
                'cargoname' => 'Other',
                    )
            );
        }
        ...
        $wpdb->insert(
                $wpdb->prefix . 'wolfvtc_users', array(
        ...
        if (isset($_GET['approve']) && intval($_GET['approve']) != 0) {
            $update = $wpdb->update(
                    $wpdb->prefix . 'wolfvtc_jobs', array(
                'approved' => 1,
        ...
        if (isset($_GET['id']) && $_GET['id'] != "") { //If a user is specified, show stuff about that user.
            ...
            if (isset($_POST['updated']) && $_POST['updated'] != "") { //If the user clicked save.
                ...
                $update = $wpdb->update(
                        $wpdb->prefix . 'wolfvtc_users', $p, array(
        ...
        if (isset($_POST['cityname'])) {
            if (strlen($_POST['cityname']) >= 1 && strlen($_POST['cityname'] <= 70)) {
                $qu = $wpdb->insert(
                        $wpdb->prefix . 'wolfvtc_cities', array('cityname' => $_POST['cityname']), array('%s')
                );
        ...
        if (isset($_POST['cargoname'])) {
            if (strlen($_POST['cargoname']) >= 1 && strlen($_POST['cargoname'] <= 35)) {
                $qu = $wpdb->insert(
                        $wpdb->prefix . 'wolfvtc_cargo', array('cargoname' => $_POST['cargoname']), array('%s')
                );
